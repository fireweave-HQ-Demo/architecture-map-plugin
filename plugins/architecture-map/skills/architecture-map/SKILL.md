...
| `RUNTIME/modes/repo.ts` | Drafts the `repo` map from the inventory (what `init` writes) |
| `RUNTIME/modes/plan.ts` | `membershipOf` + `mechanicalPlanIssues`: statuses disk can contradict |
| `RUNTIME/modes/pr.ts` | `mergePrHops(before, after)` → unchanged / added / removed / changed |
| `RUNTIME/render/inject.ts` · `render/shell.html` | JSON → interactive HTML. The shell is locked. |
